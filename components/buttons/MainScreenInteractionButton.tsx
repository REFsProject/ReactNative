...
export default function MainScreenInteractionButton(props: InteractionButtonProps, className?: string): React.JSX.Element
{

    let name = className || "mr-0 ml-auto";

    let color = props.focusState.focus === props.componentName.toString() ? "orange" : "white";

    const defaultOnPress = (): void => {
        console.log(props.focusState.focus);
        if(props.focusState.focus !== props.componentName.toString()){
            props.focusState.setFocus(props.componentName.toString());
            return;
        }
        props.focusState.setFocus("");
    }


    return (
        <View className={name} style={{paddingVertical: 10, width: 70}}>
            <TouchableOpacity className={"flex-col justify-center items-center border"} style={{paddingHorizontal: 10, width: 70, display:"flex", alignContent:"center"}} onPress={props.onPress && defaultOnPress}>
                {props.componentName !== undefined ? <AntDesign style={{marginLeft:3}} name={props.componentName} size={30} color={color} /> : <Image source={{...props.image}}/>}
                {props.data !== undefined ? <Text style={{color: color, textAlign: "auto", fontSize: 10}}>{props.data}</Text> : null}
            </TouchableOpacity>
        </View>
    )
}